...
class transformerencoder_maml_spkemb_mask_02(nn.Module):
    def __init__(self,):
        super(transformerencoder_maml_spkemb_mask_02,self).__init__()
        ...
        for i in range(num_hidden_layers):
            layer.append(encoder_layer(num_convs[-1], size_per_head, num_heads, dropout_rate, intermediate_size))
        self.encoder = nn.Sequential(*layer)
        self.linear1 = nn.Linear(192,257)
        self.linear2 = nn.Linear(257,257)
        self.linear3 = nn.Linear(257,257)
        self.leakyrelu = nn.LeakyReLU(negative_slope=0.3, inplace=False)
    ...
    def forward(self,x,emb,mm="n"):
        #x = self.first_layer(x.permute(0,2,1))
        if mm == "spk":
            #pdb.set_trace()
            emb = emb[0].unsqueeze(-1).repeat(1,1,x.shape[1])
            emb1 = self.leakyrelu(self.linear1(emb.permute(0,2,1)))
            emb2 = self.leakyrelu(self.linear2(emb1))
            emb_mask = self.sigmoid(self.linear3(emb2))
            #emb_mask = self.leakyrelu(self.linear3(emb2)) # try LReLU
            x_masked = x*emb_mask#*0.3 + x*self.sigmoid(emb1)*0.35+x*self.sigmoid(emb2)*0.35
        x = self.first_layer((x_masked).permute(0,2,1))
        x = self.encoder(x.permute(0,2,1))
#         print(x.shape)
        x = self.last_layer(x)
#         print(x.shape)
        #pdb.set_trace()
        return x#, emb1, x_masked       
